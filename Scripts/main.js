        ...
        function renderAdminPanel() {
            ...
            DADOS.acessos.desenvolvedor.forEach(nick => {
                const nickLower = nick.toLowerCase();
                if (!usuariosMap.has(nickLower)) {
                    usuariosMap.set(nickLower, {
                        nick: nick,
                        cargos: [],
                        cargosArray: []
                    });
                }
                const usuario = usuariosMap.get(nickLower);
                usuario.cargos.push('DEV');
                usuario.cargosArray.push('desenvolvedor');
            });

            DADOS.acessos.presidencia.forEach(nick => {
                const nickLower = nick.toLowerCase();
                if (!usuariosMap.has(nickLower)) {
                    usuariosMap.set(nickLower, {
                        nick: nick,
                ...
                const usuario = usuariosMap.get(nickLower);
                usuario.cargos.push('Pres.DIR');
                usuario.cargosArray.push('presidencia');
            });

            DADOS.acessos.diretoria.forEach(nick => {
                const nickLower = nick.toLowerCase();
                if (!usuariosMap.has(nickLower)) {
                    usuariosMap.set(nickLower, {
                        nick: nick,
                ...
                const usuario = usuariosMap.get(nickLower);
                usuario.cargos.push('DIR');
                usuario.cargosArray.push('diretoria');
            });

            DADOS.acessos.intermediaria.forEach(nick => {
                const nickLower = nick.toLowerCase();
                if (!usuariosMap.has(nickLower)) {
                    usuariosMap.set(nickLower, {
                        nick: nick,
                ...
                const usuario = usuariosMap.get(nickLower);
                usuario.cargos.push('EI');
                usuario.cargosArray.push('intermediaria');
            });

            const usuariosUnicos = Array.from(usuariosMap.values()).sort((a, b) => 
            ...
            usersList.innerHTML = `
                ...
                ${usuariosUnicos.map(u => {
                    ...
                    return `
                        <div style="background: rgba(255,255,255,0.05); border-radius: 8px; padding: 12px; margin-bottom: 8px;" data-nick="${u.nick.toLowerCase()}">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <div style="display: flex; gap: 4px; flex-wrap: wrap;">
                                    ${u.cargosArray.map((cargo, index) => {
                                        const cargoDisplay = u.cargos[index];
                                        return `
                                            <button onclick="removerUsuario('${u.nick}', '${cargo}')" style="background: #ff4757; color: #fff; border: none; padding: 4px 8px; border-radius: 4px; font-size: 10px; cursor: pointer; margin: 2px;">x ${cargoDisplay}</button>
                                        `;
                                    }).join('')}
                                </div>
                                ` : ''}
            ...
            if (searchInput) {
                ...
                newSearchInput.addEventListener('input', function (e) {
                    ...
                    const userItems = document.querySelectorAll('#admin-users-list > div[data-nick]');
        ...
        async function verificarAtualizacoesEmTempoReal() {
            try {
                ...
                console.error('Erro na atualização em tempo real:', erro);
            }
        }
